...
public class ActionBar extends Bar {
    ...
    private void drawTowerCost(Graphics g) {
        ...
        //Show this if player lacks gold for selected towers.
    ...
    private void drawEnemiesLeftInfos(Graphics g) {
    ...
    private void drawDisplayedTower(Graphics g) {
        if(displayedTower != null){
            ...
            if(sellTower.isMouseOver()){
                g.setColor(Color.RED);
                g.drawString("Sell for: "+getSellAmount(displayedTower) + "g", 490, 695);
            }else if(upgradeTower.isMouseOver() && gold >= getUpgradeAmount(displayedTower)){
                g.setColor(Color.blue);
                g.drawString("Upgrade for: "+getUpgradeAmount(displayedTower) + "g", 490, 695);
            }
        }
    }

    private int getUpgradeAmount(ATower displayedTower) {
        return (int)(helpz.Constants.Towers.GetTowerCost(displayedTower.getTowerType()) * 0.3f);
    }

    private int getSellAmount(ATower displayedTower) {
    ...
    public void displayedTower(ATower t){
        displayedTower = t;
    }
        ...
        if( bMenu.getBounds().contains(x,y)){
            ...
            for(MyButton b : towerButtons){
                if (b.getBounds().contains(x,y)){
                    ...
                    //selectedTower = new ATower(0, 0, -1, b.getId());
                    selectedTower=createTowerInstance(b.getId());
    ...
    private ATower createTowerInstance(int towerType){
        switch (towerType) {
            case 0:
                return new Cannon(0, 0, -1, towerType);
            case 1:
                return new Archer(0, 0,-1, towerType);
            case 2:
                return new Wizard(0, 0,-1, towerType);
            default:
                return null;
        }
    }
